...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | --------------- | ----------------------------------------- |
| 2   | - **Learning Content:** <br>&emsp; + AWS CLI Overview: Command-line tool to manage AWS from the terminal, great for automation & DevOps. Each CLI command maps to an AWS API. <br>&emsp; + IAM user & credentials: It's mandatory to use an IAM user (Access/Secret keys), never root. Credentials are saved locally for authentication. <br>- **Project - Smart Media Analysics (Frontend):** <br>&emsp; + Integrate Search API via React Query to call `POST /api/v1/search` endpoint. <br>&emsp; + Synchronize search states (query, media_type, tags) with URL Search Params. | 06/18/2026 | 06/18/2026      | <https://000011.awsstudygroup.com/> |
| 3   | - **Learning Content:** <br>&emsp; + CLI Configuration: Use `configure` to set Access keys, region, and format. CLI automatically reads credentials from the local config directory. <br>&emsp; + Basic Commands: Check version, IAM identity (`sts get-caller-identity`); S3 ops (list, create, delete); EC2 ops (list regions/instances); IAM ops (list users/policies). <br>- **Project - Smart Media Analysics (Frontend):** <br>&emsp; + Handle Abort Requests to prevent race conditions and lock UI components during Loading. <br>&emsp; + Implement Empty Query Validation, manage error handling messages, and build Search History. | 06/19/2026 | 06/19/2026      | <https://000011.awsstudygroup.com/> |
